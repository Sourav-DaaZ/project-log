  ...
  (exports.IsPresent = (array1, array2) => {
    try {
      ...
      for (const prop in array1) {
        if (!(typeof array1[prop] === "string" && array1[prop].length === 0))
          val.push(prop);
      }
...
exports.paginationData = async (data, pageCount) => {
  try {
    ...
    paginationData = await data.slice(
      config.pageDataCount * page,
      config.pageDataCount * (page + 1)
    );
...
exports.paginationChatData = (data, pageCount) => {
  try {
    ...
    let paginationData = [];
    const startPage =
      arrSize - config.pageChatCount > 0 ? arrSize - config.pageChatCount : 0;
  ...
  try {
    if (data.encritption && data.data) {
      const bytes = CryptoJS.AES.decrypt(
        data.data.toString(),
        config[config.env].apiEncryptionSecret
      );
...
const apiEncryptionData = (data) => {
  try {
    const cipherDoc = CryptoJS.AES.encrypt(
      JSON.stringify(data),
      config[config.env].apiEncryptionSecret
    ).toString();
    const varData = {
  ...
  try {
    if (isUpadted) {
      if (typeof name === "string") {
        uniq_name = name
          ?.toString()
          .slice(name.toString().lastIndexOf("/") + 1);
